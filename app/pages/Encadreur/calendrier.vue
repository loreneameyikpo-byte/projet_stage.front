...
<template>
  <div>
    ...
    <p class="text-sm text-ink-light mt-1 mb-6">Vos soutenances, comme encadrant ou membre de jury</p>

    <div class="grid lg:grid-cols-3 gap-6">
      <!-- Calendrier -->
      <div class="lg:col-span-2 bg-card border border-slate-200 rounded-lg p-5">
        ...
        <div class="grid grid-cols-7 gap-px bg-slate-100 rounded-lg overflow-hidden text-xs">
          ...
          <div v-for="(jour, i) in joursDuMois" :key="i" class="bg-white min-h-[80px] p-1.5" :class="!jour ? 'bg-slate-50/50' : ''">
        ...
        <div class="bg-card border border-slate-200 rounded-lg p-5">
          ...
          <div class="space-y-3">
            <button
              ...
              :key="p.id"
              type="button"
              class="w-full text-left border border-slate-100 rounded-lg p-3 hover:border-secondary/40 transition"
              @click="presentationDetail = p"
            >
