...
const { data, refresh } = await useAsyncData<{ utilisateurs: Encadreur[] }>('encadreurs', () =>
...
const encadreursFiltres = computed(() => {
  const terme = recherche.value.toLowerCase()
  return (data.value?.utilisateurs ?? []).filter((e) =>
    `${e.nom} ${e.prenom} ${e.email} ${e.specialite ?? ''}`.toLowerCase().includes(terme)
  )
})

function initiales(e: Encadreur) {
...
const encadreurEnEdition = ref<Encadreur | null>(null)

function ouvrirCreation() {
  encadreurEnEdition.value = null
  modaleOuverte.value = true
}
function ouvrirEdition(e: Encadreur) {
  encadreurEnEdition.value = e
  modaleOuverte.value = true
}
...
async function supprimer(e: Encadreur) {
  const confirme = await demander({
    titre: 'Supprimer cet encadreur',
...
function ligne(v: Record<string, unknown>): Encadreur {
  return v as unknown as Encadreur
}
...
<template>
  <div>
    <div class="flex items-start justify-between mb-6 opacity-0" :class="estMonte ? 'animate-entree' : ''">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Gestion des encadreurs</h1>
        <p class="text-sm text-ink-light mt-1">{{ data?.utilisateurs.length ?? 0 }} encadreurs enregistrés</p>
      </div>
      <button
        type="button"
    ...
    <div class="bg-card border border-slate-200 rounded-t-lg overflow-hidden opacity-0 datatable-projetis" :class="estMonte ? 'animate-entree' : ''" style="animation-delay: 140ms">
      <Vue3Datatable
        :rows="encadreursFiltres as unknown as Record<string, unknown>[]"
        :columns="colonnes"
        :pagination="true"
        ...
        skin="bh-table-hover"
        pagination-info="{0} à {1} sur {2} encadreurs"
      >
        <template #encadreur="data">
          <div class="flex items-center gap-2.5">
            <span class="badge-avatar w-8 h-8 rounded-full bg-accent/10 text-accent text-xs font-semibold flex items-center justify-center shrink-0">
        ...
        <template #specialite="data">
          <span v-if="ligne(data.value).specialite" class="badge-filiere inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-accent/10 text-accent">
        ...
        <template #noData>
          <p class="px-5 py-10 text-center text-ink-light text-sm">Aucun encadreur trouvé.</p>
        </template>
    ...
    <Transition name="modale-fondu">
      <ModaleUtilisateur
        v-if="modaleOuverte"
        type="encadreur"
        :utilisateur="encadreurEnEdition"
        :id-role="rolesMap?.encadreur ?? ''"
